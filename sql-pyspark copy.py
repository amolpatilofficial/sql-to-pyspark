...
def fn_createtable(query):
    v_parse = parse(query)
    v_createtable = v_parse["createtable"]
    return v_createtable
...
def fn_genSQL(data):
    ...
    for key, value in data.items():
        ...
        #handle create table  
        if str(key) == "createtable":
            v_fn_createtable = fn_createtable(query)
    ...
    if v_fn_createtable:
        v_final_stmt = v_final_stmt + "\n.createtable(" + str(v_fn_createtable) + ")"
